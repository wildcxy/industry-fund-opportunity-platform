...
def main() -> None:
    ...
    job_name = "fund-metrics-ingestion"
    ...
    try:
        archived_path, row_count = archive_dropzone_file(
            "fund-daily-metrics", batch_id, trade_date, "daily/fund_daily_metrics.csv"
        )
        status = "success" if archived_path else "skipped"
        print(f"Fund metrics archived: path={archived_path} row_count={row_count}")
