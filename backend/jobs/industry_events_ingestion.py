...
def main() -> None:
    ...
    job_name = "industry-events-ingestion"
    ...
    try:
        archived_path, row_count = archive_dropzone_file(
            "industry-events", batch_id, trade_date, "daily/industry_events.csv"
        )
        status = "success" if archived_path else "skipped"
        print(f"Industry events archived: path={archived_path} row_count={row_count}")
